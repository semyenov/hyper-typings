...
export class WorkspaceIndex extends BaseWorkspaceCore {
  constructor(
    ...
    public secretKey: Buffer | undefined,
    public isOwnerIndex: boolean,
  ) {
  ...
  static load(
    ...
    secretKey: string | Buffer | undefined,
...
export class Workspace {
  ...
  constructor({ store, writers, indexes, swarmKeyPair }: WorkspaceOpts) {
    ...
    this.autobase = new Autobase(this.store, {
      outputs: indexes.map((idx) => idx.core),
    })

    this.indexCore = Autobase.getLocalCore(this.store)
  ...
  [Symbol.for('nodejs.util.inspect.custom')](depth: number, opts: any) {
    ...
    const inspectWsWriter = (w: WorkspaceWriter) => {
      return (
        ...
        indent +
        '      key: ' +
        opts.stylize(toHex(w.publicKey), 'string') +
        '\n' +
        indent +
        '      name: ' +
        opts.stylize(w.name, 'string') +
        '\n' +
        indent +
        '      admin: ' +
        opts.stylize(w.isAdmin, 'boolean') +
        '\n' +
        indent +
        '      owner: ' +
        opts.stylize(w.isOwner, 'boolean') +
        '\n' +
        indent +
        '      writable: ' +
        opts.stylize(w.core.writable, 'boolean') +
        '\n' +
        indent +
        '    }\n'
      )
    ...
    const inspectWsIndex = (idx: WorkspaceIndex) => {
      return (
        ...
        indent +
        '      key: ' +
        opts.stylize(toHex(idx.publicKey), 'string') +
        '\n' +
        indent +
        '      writable: ' +
        opts.stylize(idx.core.writable, 'boolean') +
        '\n' +
        indent +
        '    }\n'
      )
    ...
    return (
      ...
      indent +
      '  admin: ' +
      opts.stylize(this.isAdmin, 'boolean') +
      '\n' +
      indent +
      '  owner: ' +
      opts.stylize(this.isOwner, 'boolean') +
      '\n' +
      indent +
      '  swarmPubKey: ' +
      opts.stylize(toHex(this.swarmKeyPair.publicKey), 'string') +
      '\n' +
      indent +
      '  writers: [\n' +
  ...
  static async load(
    ...
    const ownerWriter = await WorkspaceWriter.load(
      store,
      publicKey,
      undefined,
  ...
  async putWriter(
    key: Buffer,
    {
      ...
      admin,
      frozen,
    }: { name?: string; admin?: boolean; frozen?: boolean } = {},
  ) {
    await this._loadFromMeta() // ensure writers are fresh

    const writer = this.getMyWriter()
    if (!writer) throw new Error(`Can't modify writers: not a writer`)
    if (!writer.isAdmin && !writer.publicKey.equals(key))
      throw new Error(`Can't modify other writers: not an admin`)
    if (!writer.isAdmin && admin)
      throw new Error(`Can't modify admin settings: not an admin`)
    if (!writer.isAdmin && frozen)
      throw new Error(`Can't modify frozen settings: not an admin`)
    await this.autobase.append(
      WorkspaceWriter.packop({
  ...
  async useInvite(invite: string) {
    if (!this.wireExt.writerCtrl)
      throw new Error(
        `Unable to access invite protocol. Is the owner index loaded?`,
      )
    if (this.getMyWriter())
      throw new Error(`Can't use invite: already a writer`)

    const writer = await WorkspaceWriter.createNew(this.store, {
      isOwner: false,
      isAdmin: false,
    })
    await writer.core.ready()
  ...
  async _getIndexedFile(
    path: string,
  ): Promise<structs.IndexedFile | undefined> {
  ...
  _gatherIndexedFileChangeParents(
    indexedFile: structs.IndexedFile | undefined,
  ) {
    let parents: string[] = []
    if (indexedFile) {
  ...
  async _getIndexedChangeParents(path: string): Promise<string[]> {
    return this._gatherIndexedFileChangeParents(
      await this._getIndexedFile(path),
    )
  }

  async _getIndexedNomergeParents(
    path: string,
    writer: Buffer,
  ): Promise<string[]> {
  ...
  async _getFileInfo(
    indexedFile: structs.IndexedFile,
  ): Promise<structs.FileInfo> {
    ...
    const otherChanges: structs.IndexedChange[] = (
      indexedFile.otherChanges?.length > 0
        ? await Promise.all(
            indexedFile.otherChanges.map((c) => this.getChange(c)),
          )
        : []
    ).filter(Boolean)
    return {
  ...
  async readAllFileStates(
    path: string,
  ): Promise<{ writer: Buffer; data: Buffer }[]> {
    await this.indexCore.update()
    const indexedFile = await this._getIndexedFile(path)
    if (!indexedFile) return []
    ...
    for (const changeId of indexedFile.otherChanges) {
      ...
      if (change && (change.details as structs.ChangeOpPut).blob) {
        buffers.push({
          writer: indexedFile.writer,
          data: (await this._getBlobData(
            (change.details as structs.ChangeOpPut).blob,
          )) as Buffer,
        })
      }
    }
  ...
  async getChange(
    changeId: string,
  ): Promise<structs.IndexedChange | undefined> {
    await this.indexCore.update()
  ...
  async listHistory(opts?: any): Promise<structs.IndexedChange[]> {
    ...
    const self = this
    const matcher =
      typeof opts?.path === 'string' ? match.matcher(opts.path) : undefined
    return await new Promise((resolve, reject) => {
      pump(
          ...
          self.getChange(entry.value).then(
            (change) => {
              if (change) {
                if (
                  matcher &&
                  (!('path' in change.details) || !matcher(change.details.path))
                ) {
                  // skip
                } else {
                  this.push(change)
  ...
  async _loadFromDeclaration(core: Hypercore): Promise<void> {
    ...
    const ownerIndex = WorkspaceIndex.load(
      this.store,
      declOp.index,
      undefined,
      true,
  ...
  async _loadFromMeta(meta?: structs.IndexedMeta): Promise<void> {
    ...
    if (!meta) {
      ...
      meta = structs.isIndexedMeta(metaEntry?.value)
        ? metaEntry.value
        : undefined
    }
    if (!meta) return
      ...
      try {
        ...
        if (structs.isDeclareOp(op)) {
          ...
          if (owner && change.equals(owner.publicKey)) {
            ...
            console.error(
              'Error: declaration operation found on non-owner core, key:',
              change,
              'op:',
              op,
            )
  ...
  async _applyChangeOpFileAct(op: structs.ChangeOp, b: any, change: Buffer) {
    const pathp = (op.details as structs.ChangeOpFilesAct).path
      .split('/')
      .filter(Boolean)
    if (pathp.length === 0) {
      console.error(
        `Invalid path "${(op.details as structs.ChangeOpFilesAct).path}", skipping operation`,
        op,
      )
      return
    }
    ...
    const currIndexedFile = structs.isIndexedFile(currIndexedFileEntry?.value)
      ? currIndexedFileEntry.value
      : undefined
    ...
    const otherChanges = currParents.filter(
      (parent) => !op.parents.includes(parent),
    )
  ...
  async _applyChangeOpMetaAct(op: structs.ChangeOp, b: any, change: Buffer) {
    ...
    const currIndexedMeta: structs.IndexedMeta = structs.isIndexedMeta(
      currIndexedMetaEntry?.value,
    )
      ? currIndexedMetaEntry.value
      : undefined
    if (!currIndexedMeta) {
      console.error(
        'Unable to update writers, _meta entry invalid. Entry:',
        currIndexedMetaEntry,
      )
      return
    }
    ...
    if (
      !writer ||
      (!writer.isAdmin && !putWriterDetails.key.equals(writer.publicKey))
    ) {
      console.error(
        'Non-admin attempted to edit writers, key:',
        change,
        'op:',
        op,
      )
    ...
    const existingEntry = currIndexedMeta.writers.find(
      (w: structs.IndexedMetaWriter) => w.key.equals(putWriterDetails.key),
    )
    if (existingEntry) {
      if ('name' in putWriterDetails)
        existingEntry.name = isString(putWriterDetails.name)
          ? putWriterDetails.name
          : existingEntry.name
      if (hasAdminPowers && 'admin' in putWriterDetails)
        existingEntry.admin = isBoolean(putWriterDetails.admin)
          ? putWriterDetails.admin
          : existingEntry.admin
      if (hasAdminPowers && 'frozen' in putWriterDetails)
        existingEntry.frozen = isBoolean(putWriterDetails.frozen)
          ? putWriterDetails.frozen
          : existingEntry.frozen
    } else {
