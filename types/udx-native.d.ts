declare module 'udx-native' {
	...
	} & AsyncIterable<Array<{name: string; host: string; family: 4 | 6; internal: boolean}>>;
	...
	export class UDXSocket {
		...
		sendSync(message: Buffer, {host, family, port}: {host: string; family: 4 | 6; port: number}): void;
		on(event: 'close', listener: () => void): this;
	...
	export class UDXStream extends Duplex {
		...
		connect(socket: UDXSocket, remoteId: number, remotePort: number, remoteHost?: string, options?: {ack?: boolean}): Promise<void>;
		reconnect(remotePort: number, remoteHost?: string, options?: {ack?: boolean}): Promise<void>;
	...
	export default class UDX {
		...
		getNetworkInterfaces(): Array<{name: string; host: string; family: 4 | 6; internal: boolean}>;
		watchNetworkInterfaces(): NetworkInterfacesWatcher;
		resolveHost(host: string, options?: {family?: 0 | 4 | 6}): {host: string; family: 4 | 6};
	}
}